/**
 * FTM Studio — Cloudflare Pages Worker (_worker.js)
 * Handles: Stripe Checkout sessions + static asset serving
 *
 * Environment variables — add these in Cloudflare Pages:
 *   Pages → ftm-studio → Settings → Environment variables → Add variable (as Secret):
...
export default {
  async fetch(request, env) {
    ...
    // ── Everything else → Cloudflare Pages serves the static files ──
...
async function handleCheckout(request, env) {
  try {
    ...
    // Shows ALL payment methods you've enabled in your Stripe dashboard
    // (Card, Klarna, Clearpay, PayPal — enable them in Stripe → Settings → Payment methods)
